        ...
        for(int iter_U = 0; iter_U < in.U_iterations; iter_U++)
        {
            ...
            for(int h = 0; h < in.h_increment+1; h++)
            {
                ...
                for(int mu_iter =0; mu_iter< in.x_iterations; mu_iter++)
                {
                    ...
                    set_matrix_real(Lud, 5.37196202); //
                    set_matrix_real(Ldu, 5.37196202); //
                    aSTu = -2.04601872; // * 2.78528 / 2.0;
                    aSTd = -2.04601872; // * 2.78528 / 2.0;
                    nT = 1;
...
void calculate_Lm(double & Lm, double U, double fGG, double Lp, double Xp)
{
    double LmLm = ( U - Lp + fGG * Xp * Lp * Lp * Lp * (1.0 + conj(fGG) * conj( Lp )  ) )
                                                            /
                                             ( - conj(fGG) * Xp * Lp * (1.0 - Lp * fGG )  );
...
struct Xp_implicit
{
    Xp_implicit(int p0, double y0, double y1, double y2, AWT & g1, AWT & g2, AWT & g3, AWT & g4,
                AWT & z0, AWT & z1, AWT & z2, AWT & z3, AWT & z4) :
    print(p0), Lp(y0), U(y1), fGG(y2), GdGu1pp(g1), GdGu1pm(g2), GdGu2pm(g3), GdGu2mm(g4),
    GTu(z0), GTd(z1), FD(z2), BE(z3), aux1(z4)  { }

    double operator()(double Xp)
    {
        if(print ==1)   cout << endl << "Iter Xp: " << Xp << endl;

        double Lm=0;
        calculate_Lm(Lm, U, fGG, Lp, Xp);


        if(print ==1)   cout << "iter Lm: " << Lm << endl;


        AWT D;
        D.initializeAWT(GTu.n, GTu.xMax, GTu.kT);

        // positive Dud frequencies
        for(int j = 0;          j < D.n+1;    j++)
        {
            D.y[j] = 1.0 + GdGu1pp.y[j] * Lp
                         - GdGu1pm.y[j] * Lm
                         + GdGu2pm.y[j] * Lm
                         - GdGu2mm.y[j] * conj(Lp)
                         + GdGu1pp.y[j] * GdGu2mm.y[j] * ( Lm * conj(Lm) - Lp * conj(Lp)  );
        }
        // zero padding of Dud
        for(int j = D.n+1;    j < 3*D.n+4;  j++)
        {
            D.y[j] = 0;
        }
        // negative Dud frequencies
        for(int j = 3*D.n+4;  j < 4*D.n+4;  j++)
        {
            D.y[j] = 1.0 + GdGu1pp.y[j] * Lp
                         - GdGu1pm.y[j] * Lm
                         + GdGu2pm.y[j] * Lm
        ...
        calculate_FDud(FDud, GTd, GTu, D, FD, aux1, 0);
        calculate_BDud(BDud, GTd, GTu, D, BE, aux1, 0);

        if(print ==1)   cout << "iter min of X(+): " << Xp - real(FDud.pp + BDud.pp) << endl;

        return  abs( Xp - real(FDud.pp + BDud.pp) );

    }

    int print;
    double Lp, U, fGG;
    AWT & GTu, & GTd, & GdGu1pp, & GdGu1pm, & GdGu2pm, & GdGu2mm;
    AWT & FD, & BE;
    AWT & aux1;
};


double Brent_Xp(int print, double U, double Lp, double fGG, AWT & GdGu1pp, AWT & GdGu1pm, AWT & GdGu2pm, AWT & GdGu2mm,
                AWT & GTu, AWT & GTd, AWT & FD, AWT & BE, AWT & K3, AWT & aux1)
{
    ...
    pair<double, double> Xp_brent = brent_find_minima( Xp_implicit(print, Lp, U, fGG, GdGu1pp, GdGu1pm, GdGu2pm, GdGu2mm, GTu, GTd, FD, BE, aux1), Xp_min, Xp_max, bits);

    cout << endl << "Xp is: " << Xp_brent.first << endl;

    return Xp_brent.first;
}
...
void calculate_Xp_right(double & Xp_right, double Xp, double Lm, double U, double fGG, double Lp,
                        AWT & GTu, AWT & GTd, AWT & FD, AWT & BE, AWT & aux1,
                        AWT & GdGu1pp, AWT & GdGu1pm, AWT & GdGu2pm, AWT & GdGu2mm)
{
     calculate_Lm(Lm, U, fGG, Lp, Xp);
        ...
        //if(print ==1)   cout << "iter min of X(+): " << Xp - real(FDud.pp + BDud.pp) << endl;
        Xp_right = real(FDud.pp + BDud.pp);
}


void static_Lp(string input)
{
    // constants
    double Pi = 3.14159265359;
    complex<double> unity(0,1.0);
   ...
   zero.setZero();
    ...
    // setting the initial value of Lud vertex
    set_matrix_real(   Lud, in.U_min);
    ...
    for(int iter_kT = 0; iter_kT < in.kT_iterations; iter_kT++)
    {
        ...
        for(int iter_U = 0; iter_U < in.U_iterations; iter_U++)
        {
            ...
            for(int h = 0; h < in.h_increment+1; h++)
            {
                double hfield = hfield + h*in.h_increment;
                    ...
                    for(int mu_iter =0; mu_iter< in.x_iterations; mu_iter++)
                    {
                        ...
                        double mu = in.x_min + mu_iter * in.x_increment + U / 2.0;

                        // subdirectories for different U calculations are created
                        string     U_string =  double_to_string(U,      in.U_precision);
                        string     h_string =  double_to_string(hfield, 2);
                        string     x_string =  double_to_string(  mu - U / 2.0, 2);
                        string     kT_string =  double_to_string(  kT, 4);

                        string input_string;

                        string dirname = "Txt/U" + U_string + "_h" + h_string + "_x" + x_string
                                            + "_kT" + kT_string +  input_string + "_1D_Xp";
                        createDirectories(dirname);

                        welcome_screen(U, h, mu, kT, dirname);
                        ...
                        // 0. step: setting the initial GTd, GTu
                        //          calculate bubbles
                        freePropagatorLorentzShift(in.delta, mu, STd, GTd);
                        freePropagatorLorentzShift(in.delta, mu, STu, GTu);


                        cout << "Calculating GdGu1pp, GdGu1pm, GdGu2pm, GdGu2mm, fGG" << endl;
                        calculateGGud(GdGu1pp, GdGu1pm, GdGu2pm, GdGu2mm, GTd, GTu, FD, K3, help, 0);

                        //double fGG_exact = -0.159155;

                        complex<double> fGG;
                        calculate_fGG(fGG, GTd, GTu, FD, aux1);
                        cout << fGG << endl << endl;

                        double fGG_exact = real(fGG);

                      for(int iter_Lp = 0; iter_Lp < 150; iter_Lp++)
                        {
                            double Lp, Lm, Xp, Xm, Km;

                            Lp =  2.80 + iter_Lp * 0.05;

                            data_output << endl <<  Lp << endl;

                            string  Lp_string = double_to_string( Lp, 2);

                            name = dirname + "/data_" +  Lp_string;
                            data_output.open(name, std::ios_base::app);



                            for(int iter_Xp = 0; iter_Xp < 301; iter_Xp++)
                            {
                                double Xp = -1.00 + 0.01*iter_Xp;
                                calculate_Lm(Lm, U, fGG_exact, Lp, Xp);


                                double Xp_right;

                                calculate_Xp_right(Xp_right, Xp, Lm, U, fGG_exact, Lp,
                                                    GTu, GTd, FD, BE, aux1,
                                                    GdGu1pp, GdGu1pm, GdGu2pm, GdGu2mm);
                                ...
                                calculate_Dud_static(Dud, GdGu1pp, GdGu1pm, GdGu2pm, GdGu2mm, Lud, 0);

                                calculate_BDud(BDud, GTd, GTu, Dud, BE, aux1, 0);
                                calculate_FDud(FDud, GTd, GTu, Dud, FD, aux1, 0);

                                Xm = real(BDud.mp + FDud.mp);

                                calculate_Kud_static(Kud, GdGu1pp, GdGu1pm, GdGu2pm, GdGu2mm, Lud, 0);
                                Km = real(Kud.pm);

                                double Lp_right =  + U + fGG_exact * Xp_right * Lp * Lp * Lp * (1.0 + fGG_exact * Lp)
                                            - (-fGG_exact * Xp_right * Lp ) * ( 1.0 - Lp * fGG_exact) * U * U / (1.0 + Km * Xm)  / (1.0 + Km * Xm);


                                data_output << Xp << "   " << Lm << " " << Xp - Xp_right << " " << Lp - Lp_right << endl;
                                cout << Xp << " " << Lm << " " << Xp - Xp_right << " " << Lp - Lp_right << endl;
...
void static_spins_specs(string input)
{
    // constants
    double Pi = 3.14159265359;
    complex<double> unity(0,1.0);
            ...
            for(int h = 0; h < in.h_increment+1; h++)
            {
                ...
                for(int mu_iter =0; mu_iter< in.x_iterations; mu_iter++)
                {
                    ...
                    while( Lambda <= U )
                    {
                        ...
                        string lambda_string =  double_to_string(  Lambda, 3);
                        //string dirname = "Txt/U" + U_string + "_h" + h_string + "_x" + x_string + "_kT" + kT_string + "_L" + lambda_string;
                        string dirname = "Txt/NSS/U" +U_string + "_L" + lambda_string;
                        createDirectories(dirname);
                        ...
                        set_matrix_real(Lud, Lambda); //
                        set_matrix_real(Ldu, Lambda); //
                        aSTu = 0; // * 2.78528 / 2.0;
                        aSTd = 0; // * 2.78528 / 2.0;
                        nT = 1;
                        ...
                        if(in.model.compare("SIAM") == 0)
                        {
                            // ordinare free propagators of SIAM
                            freePropagatorLorentzShift(in.delta, mu, 0, GTd);
                            freePropagatorLorentzShift(in.delta, mu, 0, GTu);
                        }

                        if(in.model.compare("SSN") == 0)
                        {
                            // propagators of SSN set-up after rotation
                            freeNssPropagatorLorentz(in.delta, mu, in.gap, in.gammaS, in.phi, GTd);
                            freeNssPropagatorLorentz(in.delta, mu, in.gap, in.gammaS, in.phi, GTu);
                        }


                        // we set a propagator function which corresponds to SSN set-up
                        ...
                        name = dirname + "/sigma";
                        SSu.exportAWTasFUN(name, in.display, 10, 10, in.output_mode);
                        ...
                        //while( ( abs(nS - nS_old) > 0.000001 ) && iter_nS < 1000 )
                        {
                            ...
                            propagatorNssLorentzShift(in.delta, mu + U / 2.0 - U * nS / 2.0, in.gap, in.gammaS, in.phi, - hfield + aSTu, GSu, SS0);
                            propagatorNssLorentzShift(in.delta, mu + U / 2.0 - U * nS / 2.0, in.gap, in.gammaS, in.phi, + hfield - aSTd, GSd, SS0);
                        ...
                        name = dirname + "/green";
                        GSu.exportAWTasFUN(name, 10, 10, 1, in.output_mode);
